...
const AnimatedLink = ({ title, href }: { title: string; href: string }) => {
  ...
  return (
    <Link href={href} passHref>
      ...
      >
        <AnimatedWord title={title} animations={letterAnimation} isHovered={isHovered} />
        <AbsoluteContainer>
          <AnimatedWord title={title} animations={letterAnimationTwo} isHovered={isHovered} />
        </AbsoluteContainer>
      </Div>
    </Link>
  );
...
const AnimatedWord = ({
  ...
  <Word variants={titleAnimation} initial="rest" animate={isHovered ? 'hover' : 'rest'}>
